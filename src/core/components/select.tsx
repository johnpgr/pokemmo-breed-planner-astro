...
import { getPokemonSpriteUrl, parseNames, raise, randomString } from '@/lib/utils'
...
function filterPokemonByEggGroups(
    ...
    if (currentPokemon.eggTypes.includes('Genderless')) {
        ...
        return newList.concat(
            list.filter((poke) => breedable.includes(poke.name)),
        )
    }
...
export const PokemonSelect = block(
    ({
        ...
        const [currentNode, setCurrentNode] = React.useState<BreedNode | null>(
            null,
        )
            ...
            <Popover>
                <PopoverTrigger asChild>
                    <Button
                        ...
                        style={{
                            scale: NodeScaleByColorAmount[
                                String(
                                    colors.length,
                                ) as keyof typeof NodeScaleByColorAmount
                            ],
                        }}
                    >
                        {colors.map((color) => (
                    ...
                    {currentNode ? (
                        ...
                        >
                            <Button
                                size={'sm'}
                                onClick={() =>
                                    console.log(breedMap.get(position))
                                }
                            >
                                Debug
                            </Button>
                    ...
                    {!isPokemonToBreed ? (
                        <Command className="w-72 border">
                            ...
                            <div className="flex items-center gap-2 text-xs text-foreground/80 p-1">
                                ...
                                />
                                Show only {pokemonToBreed?.name}&apos;s egg
                                groups
                            </div>
                            <CommandEmpty>
                                {!pending ? 'No results' : ''}
