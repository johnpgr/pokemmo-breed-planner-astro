...
import { camelToSpacedPascal, getPokemonSpriteUrl, randomString } from '@/lib/utils'
...
export function CurrentNodeInformationCard({
    ...
    const breedPartnerPos =
        position === '0,0' ? null : getBreedingPartnerPosition(position)
    ...
    const ivDifferenceFromBreedPartner =
        IVsFromBreedPartner &&
        currentNode.ivs &&
        getIVDifferenceFromBreedPartner(currentNode.ivs, IVsFromBreedPartner)
                ...
                <CardTitle className="flex items-center">
                    {currentNode.pokemon ? (
                        <React.Fragment>
                            ...
                            <img
                                src={getPokemonSpriteUrl(currentNode.pokemon.name)}
                                style={{
                ...
                <div className="flex flex-col gap-1">
                    ...
                    {currentNode.ivs?.map((iv) => (
                        <span key={randomString(4)}>
                            31 {camelToSpacedPascal(iv)}
                        </span>
                    ))}
                </div>
                {currentNode.nature && (
                    <i className="block">{currentNode.nature}</i>
                )}
                {currentNode.pokemon ? (
                    <React.Fragment>
                        <div className="flex flex-col gap-1">
